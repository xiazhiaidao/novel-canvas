...
> **⚡ v1.17.0 流式输出+停止生成 / 上下文压缩省钱 / 导出+预算预警（2026-09-06 会话）**：用户「其他点一起优化」。① **流式+停止**：聊天与 4 个 agent 按钮消息走 SSE 流式——`POST /api/chat` 支持 `body.stream===true` → `text/event-stream`（事件 `delta`/`tool`/`done`/`error`），回复逐字上屏、工具步骤实时可见；生成中发送按钮切换为红色「停止」，点击中断：前端 `chatAbort`(AbortController)+`setChatBusy`+`chatFetchStream`(reader/TextDecoder 解析 SSE, AbortError→stopped)+`initChatStop()`(启动已绑定)，sendChat/runAgent 重写为 manual liveMsg div(dataset.done)。**⚠️ 关键坑：Node 24 undici `fetch` 在本场景 `r.body` 的 reader 卡死**(`reader.read()`/`for await` 均不返回数据,连 Promise.race 的 8s/30s timeout 都不触发;但 `r.text()` 与原生 http 正常)——`streamOneRound` 改用 **Node 原生 `http.request`/`https.request` + `data` 事件**逐块解析 SSE(处理 `reasoning_content` 思考过程不展示、`delta.content` 正文、`delta.tool_calls` 按 index 累积、末块 `usage`+`cost_cny`),彻底绕开。非流式路径(默认)完整保留。停止:客户端断开 → `res.on('close')` → `upstreamReq.destroy()` → reject 'stopped' → 回 `error: 已停止生成`。**流式 cost 不丢**:中转站把 usage+cost_cny 放流式末块,`roundRes.usage` 传入 recordUsage 正常计费(实测 streamed 调用 cost=0.0039/0.0017)。② **上下文压缩**:`compactToolResult()` 工具返回压到 4000 字符(保留 proposal_id/kind/file/isNew;read_node/read_file 留标题路径+内容截断;列表类留计数/标题);`buildAgentContext` 30000→16000;recentMessages 12000 字符预算从最新往前最多 20 条——单聊天上下文 107K tokens 量级显著下降。③ **导出+预算**:用量 tab `#usageExportBtn`「⬇ 导出 CSV」(BOM+汇总/每日/按模型/按项目/最近调用 5 段,文件名 `novel-canvas-usage-<range>-<date>.csv`);设置页 `#aiBudget`「每月预算上限(¥)」+ 用量 tab `#usageBudgetBanner`「本月预算:¥已用/¥预算(%)」,超预算红字⚠️;`/api/settings` GET 返回 `budgetPerMonth`+`monthCost`,POST 保存。验证:curl SSE delta 逐字+done;工具链 288 事件端到端;浏览器停止按钮→「(已停止生成)」;非流式正常;预算 5 保存/回读+横幅 ¥0.0221/¥5(0.4%);CSV 下载成功含按项目段。⚠️ 遗留:deepseek-v4-flash 流式首 token 约 4s(fetch 头 3.3s+生成);agent 模式长任务可能 18s+ 多轮工具;`compactToolResult` 截断可能让部分长文件工具结果不完整(计数/标题仍在,内容截断属设计)。调试用 `sse-debug.log`/`/api/ssetest` 已全部清理。

> **📚 v1.16.1 按项目统计：用量/费用/图表（2026-09-06 会话）**：用量原是全局的，recordUsage 不记项目。实现：`recordUsage(model, usage, respData, project)` 第 4 参 project；7 个调用点全传（4 个一致性功能用 `project` 参数，/api/chat 循环、/api/continue、/api/ai/edit 用 `body.project`）。新增 `byProject` 聚合（prompt/completion/total/calls/cached/cost），byChat 每条加 `project` 字段；`aggregateUsage`/`summarizeUsage` 返回 `byProject`（全量不按时间过滤）。前端用量 tab 新增「按项目（全量）」区块：项目占比环形图（conic-gradient，中心全项目总 tokens，图例色块+项目名+%）+ 7 列表格；最近调用行显示 📁项目名。⚠️ 修复 bug：`loadUsage()` 返回对象漏 `byProject` 字段导致每次只剩最近一条——已补。⚠️ 注意：历史 byChat 记录无 project（显示无 📁）；byProject 从接入时点起累计；同一项目不同写法（body.project 传「列车求生」 vs projectNameOfRoot 传文件夹名）可能被记为两个项目，需用户注意前端传参一致性。验证：真实调用后 byProject「列车求生 = 13,136 tok / ¥0.0039」、单项目环形图 100%、深色主题探针通过。

> **🌐 v1.16.0 AI 多服务商接入：国内外主流模型（2026-09-06 会话）**：用户要「大部分国内外都可以」接 AI。实现：前端 `AI_PROVIDERS` 预设表 16 家 OpenAI 兼容服务商（OpenAI/DeepSeek 官方/Gemini/Kimi/通义/智谱/豆包/文心/硅基流动/OpenRouter/零一/阶跃/混元/MiniMax/中转站/自定义），每家含默认 base + 常用模型 + 说明。设置页 AI tab 新增「服务商」下拉：选中自动填 base + 刷新模型 chips（点击填入）；按已保存 base 自动匹配（aiProviderByBase）。聊天面板 chatModel 下拉动态化：当前服务商模型置顶，自定义/未匹配展示全量 48 项；启动时把 settings base 填进 aiBase 再同步；保存设置触发 `chatModelSync` 事件重建。⚠️ TDZ 坑：AI_PROVIDERS 原定义在文件中部，而 initChatModelSelect（L4057 调用）在前 → const 未初始化 ReferenceError，已把 AI_PROVIDERS 移到文件前部（CHAT_MODEL_MIGRATIONS 前）。⚠️ Claude 官方为 Anthropic 原生格式，需 OpenRouter 等兼容网关。验证：下拉 48/2 两种状态、选中保留、服务商切换自动填 base+chips、保存联动、端到端「通了」。模型名变化快，以服务商控制台为准。
...
> **🚑 v1.15.3 「模型未返回内容」/推理模型兼容（2026-09-06 会话）**：用户报「（模型未返回内容）」。根因：中转站实测可用模型仅 `deepseek-v4-flash`（✅）与 `deepseek-v4-pro`（✅，推理模型+支持 tools）；`deepseek-chat`/`deepseek-reasoner`/`deepseek-v4-flash-vision-exp` 均 `MODEL_NOT_AVAILABLE`。且 pro 是推理模型：回复在 `reasoning_content`、`content` 为 null → server 只读 `content` 的端点（continue/ai-edit/生成下一章等）报「模型未返回内容」。修复：新增 `aiMessageContent(msg)`（content → reasoning_content 兜底）替换全部 7 处 LLM 读取点；模型下拉收敛为 flash（推荐）/ pro 两项；`CHAT_MODEL_MIGRATIONS` 补旧名迁移。验证：pro 经 chat/continue/ai-edit 均正常返回。⚠️ 遗留：测试 `/api/ai/edit` 生成的 1 个 file_edit proposal 在服务内存中，重启即清（未写盘，用户不批准不生效）。模型下拉若想加回 flash-vision 需先在中转站确认可用。
